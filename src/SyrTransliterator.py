...
class SyrTransliterator(SyrTools):
    def __init__(self):
        ...
        self.ipa_to_roman_map = {
            ...
            # Consonants
            'ʔ': "'", 'b': 'b', 'g': 'g', 'd': 'd', 'h': 'h', 'w': 'w', 
            'z': 'z', 'ḥ': 'kh', 'tˤ': 'ṭ', 'j': 'y', 'k': 'k', 'l': 'l', 
            'm': 'm', 'n': 'n', 's': 's', 'ʕ': 'ʿ', 'p': 'p', 'sˤ': 'ṣ', 
            'q': 'q', 'r': 'r', 'ʃ': 'sh',
        ...
        self.ipa_vowels = list(self.eastern_vowel_ipa_map.values()) + \
    ...
    def transliterate(self, text):
        ...
        return {'ipa': lossless_ipa_text, 'natural_ipa': phonetic_ipa_text, 'romanized': self.ipa_to_roman(phonetic_ipa_text)}
    ...
    def tokenize_cluster(self, cluster):
        token_str = self.get_subtoken_of_type(cluster, self.LETTER) + \
    ...
    def naturalize_ipa(self, ipa):
        vowels = list(self.eastern_vowel_ipa_map.values()) + list(self.mater_lectionis_ipa_map.values())
        ...
        if ipa_chars and ipa_chars[0] == glottal_stop_ipa and len(ipa_chars) > 1 and ipa_chars[1] in vowels:
        ...
        if len(ipa_chars) > 1 and ipa_chars[-1] == glottal_stop_ipa and ipa_chars[-2] in short_vowels:
            ipa_chars.pop()

        # Convert 'i' to 'ɪ' if sandwiched between two unvocalized consonants
        for j in range(1, len(ipa_chars) - 1):
            if ipa_chars[j] == self.mater_lectionis_ipa_map['ܝܼ'] and ipa_chars[j - 1] not in vowels and ipa_chars[j + 1] not in vowels:
                ipa_chars[j] = self.eastern_vowel_ipa_map[self.DOTTED_ZLAMA_HORIZONTAL]  # Change 'i' to 'ɪ'
        ...
        for char in text:
            if char in self.PUNCTUATION or char.isspace():
                if word:
                    ...
                    word = ""
                if char in self.PUNCTUATION or char.isspace():
                    result.append(self.replace_punctuation(char))  # Store punctuation as a standalone element
            else:
    ...
    def apply_bdul_prefixes(self, text):
        ...
        for tok in split_ipa:
            if len(tok) > 1 and tok[0] in self.ipa_bdol and tok[1] not in self.ipa_vowels:
    ...
    def handle_glottals(self, text):
        ...
        filtered_str = ''
        for tok in split_ipa:
            if (tok[0] == self.consonant_ipa_map['ܐ'] or tok[0] == self.consonant_ipa_map['ܑ']):
                filtered_str += tok[1:]
            elif tok[-1] == self.consonant_ipa_map['ܐ'] or tok[-1] == self.consonant_ipa_map['ܑ']:
                filtered_str += tok[:-1]
            elif ((tok[0] == self.consonant_ipa_map['ܐ'] or tok[0] == self.consonant_ipa_map['ܑ']) and
               (tok[-1] == self.consonant_ipa_map['ܐ'] or tok[-1] == self.consonant_ipa_map['ܑ'])):
        ...
        for old, new in special_phonetic_replacements:
            ...
            if old in word:
                if len(old) == len(word):
                    ...
                    if len(word) > index + len(old) and word[index + len(old)] in self.VOWEL:
                        pass
                    else:
                        word = word.replace(old, new)
    ...
    def ipa_to_roman(self, ipa_text):
        ...
        ipa_text = self.apply_bdul_prefixes(ipa_text)
            ...
            for length in [3, 2, 1]:  # Handle digraphs like "tʃ"
                if i + length <= len(ipa_text):
                    ...
                    if segment in ipa_to_syriac_map:
                        result.append(ipa_to_syriac_map[segment][0])  # Choose first if multiple
                        i += length
                        found = True
                        break
            if not found:
